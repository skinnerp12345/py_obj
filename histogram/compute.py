...
DEFAULT_EDGE_TRIM = 7  # avoids regridding-edge artifacts near the domain boundary
...
def compute_histogram(
    ...
    artifacts, e.g. regridding edge effects). Works for a plain 2D (y, x)
    obs field or a 3D (member, y, x) model field alike -- trimming only ever
    touches the last two axes, and every remaining pixel (across all
    members, if present) is ravelled into one histogram together, pooling
    all ensemble members into a single distribution.
    ...
    clip_negative_to_zero (default False): flooring every negative dBZ value
    to 0 before histogramming is a real, data-altering choice, so here it is
    an explicit opt-in rather than a silent default -- callers that want
    that behavior must ask for it.
